...
func TestIsAddKeyEnabledProvided(t *testing.T) {
	tests := []struct {
		...
		{name: "missing field defaults later", body: `{"name":"k1"}`, expected: false},
		{name: "explicit false is preserved", body: `{"name":"k1","isEnabled":false}`, expected: true},
		{name: "explicit true is preserved", body: `{"name":"k1","isEnabled":true}`, expected: true},
	}
